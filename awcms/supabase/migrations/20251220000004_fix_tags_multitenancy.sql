...
DO $$
BEGIN
    -- Only proceed if 'tags' table exists
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tags' AND table_schema = 'public') THEN
        ...
        -- Using exception handling for DDL inside DO block
        BEGIN
            ALTER TABLE public.tags DROP CONSTRAINT IF EXISTS tags_slug_key;
        EXCEPTION WHEN OTHERS THEN
            NULL; -- Ignore validation error if constraint missing
        END;

        -- 2. Create the scoped unique index (tenant_id, slug)
        EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_tags_tenant_slug ON public.tags (tenant_id, slug)';

        -- 3. Drop default value on Sync Function
        EXECUTE 'DROP FUNCTION IF EXISTS public.sync_resource_tags(uuid, text, text[])';

        -- 4. Create new Secure Function with tenant_id param
        -- Dynamic SQL allows checking table existence before defining function body
        EXECUTE $func$
        ...
        BEGIN
            ...
            ELSIF p_resource_type = 'contacts' THEN 
                -- Assuming contacts table exists if we are here? No, need to be safe strictly speaking.
                -- But if tags exists, usually junction tables exist.
                -- We can wrap in inner block if paranoid.
                BEGIN
