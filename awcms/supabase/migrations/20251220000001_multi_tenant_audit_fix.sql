...
-- Helper block for safe column addition AND RLS Policies
DO $$
DECLARE
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'contacts' AND table_schema = 'public') THEN
        ...
        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Select" ON public.contacts';
        EXECUTE 'CREATE POLICY "Tenant Isolation Select" ON public.contacts FOR SELECT TO authenticated USING (tenant_id = public.current_tenant_id())';

        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Insert" ON public.contacts';
        EXECUTE 'CREATE POLICY "Tenant Isolation Insert" ON public.contacts FOR INSERT TO authenticated WITH CHECK (tenant_id = public.current_tenant_id())';

        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Update" ON public.contacts';
        EXECUTE 'CREATE POLICY "Tenant Isolation Update" ON public.contacts FOR UPDATE TO authenticated USING (tenant_id = public.current_tenant_id())';

        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Delete" ON public.contacts';
        EXECUTE 'CREATE POLICY "Tenant Isolation Delete" ON public.contacts FOR DELETE TO authenticated USING (tenant_id = public.current_tenant_id())';
    END IF;
    ...
    -- 3. Backups
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'backups' AND table_schema = 'public') THEN
        ...
        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Select" ON public.backups';
        EXECUTE 'CREATE POLICY "Tenant Isolation Select" ON public.backups FOR SELECT TO authenticated USING (tenant_id = public.current_tenant_id())';
    END IF;

    -- 4. Backup Logs
     IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'backup_logs' AND table_schema = 'public') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cart_items' AND table_schema = 'public') THEN
        ...
        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Select" ON public.cart_items';
        EXECUTE 'CREATE POLICY "Tenant Isolation Select" ON public.cart_items FOR SELECT TO authenticated USING (tenant_id = public.current_tenant_id())';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'order_items' AND table_schema = 'public') THEN
        ...
        EXECUTE 'DROP POLICY IF EXISTS "Tenant Isolation Select" ON public.order_items';
        EXECUTE 'CREATE POLICY "Tenant Isolation Select" ON public.order_items FOR SELECT TO authenticated USING (tenant_id = public.current_tenant_id())';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'payments' AND table_schema = 'public') THEN
        ...
        EXECUTE 'CREATE INDEX IF NOT EXISTS idx_payments_tenant_id ON public.payments(tenant_id)';
    END IF;
